...
var FighterTypes = [
    ...
    },
    {
        ...
        onCollide: (f,e) => { if(f.data.latchCd<=0&&f.data.latchTimer<=0){ f.data.latchOffset=norm(f.x-e.x,f.y-e.y); f.data.latchTimer=f.type?.arsenalShell?2.5:5.0; /* Arsenal A-CORR-3 ADAPT: 2.5s latch */ f.data.latchTick=0; f.data.latchPulse=.45; if(f.isRage){ f.data.bloodLinkLevel=(f.data.bloodLinkLevel||0)+1; floatingTexts.push(new FloatingText(e.x,e.y-e.radius-66,`BLOOD LINK ${f.data.bloodLinkLevel}`,'#ff3040')); } playFighterSound(f,'skill'); } return false; },
    ...
    },
    {
        ...
        onCollide: (f,e) => { 
            ...
            if(c===1){ e.takeDamage(3,f,'monk-punch'); } 
                ...
                const RUSH_SECS=f.type?.arsenalShell?2.5:5; /* Arsenal A-CORR-3 ADAPT: 2.5s pin */ e.applyStatus('stun',RUSH_SECS,{source:f}); f.data.rushTimer=RUSH_SECS; f.data.rushTargetId=e.id; f.data.rushAnchor={x:e.x,y:e.y}; f.data.combo=0; 
